...
import junction.domain.oauth2.application.service.ReissueService;
...
public class ReissueServiceImpl implements ReissueService {
    ...
    public void reissue(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(!jwtUtil.jwtVerify(refreshToken, "refresh")) {
            ...
            throw new JunctionException(ErrorCode.INVALID_REFRESH_TOKEN);
        }

        JsonWebToken jsonWebToken = jsonWebTokenRepository.findById(refreshToken).orElse(null);

        if(jsonWebToken == null) {
            throw new JunctionException(ErrorCode.REFRESH_TOKEN_NOT_EXIST);
        }
